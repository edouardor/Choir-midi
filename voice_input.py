...
#args.input = 0
if args.input== None:
    print("No input device specified. Trying input devices now: ")
    p = pyaudio.PyAudio()
    for i in range(p.get_device_count()):
        print("Device number (%i): %s" % (i, p.get_device_info_by_index(i).get('name')), end = '')
        try:
            stream = p.open(format=pyaudio.paFloat32, channels=1, rate=44100, input=True, input_device_index=i, frames_per_buffer=4096)
        except:
            print(' Not available')
        else:
            print(' Selected')
